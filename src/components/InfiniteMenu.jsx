...
class DiscGeometry extends Geometry {
  constructor(steps = 4, radius = 1) {
    ...
    this.vertices[this.vertices.length - 1].uv[0] = 0.5
    this.vertices[this.vertices.length - 1].uv[1] = 0.5
    for (let i = 0; i < steps; ++i) {
      const x = Math.cos(alpha * i)
      const y = Math.sin(alpha * i)
      this.addVertex(radius * x, radius * y, 0)
      this.vertices[this.vertices.length - 1].uv[0] = x * 0.5 + 0.5
      this.vertices[this.vertices.length - 1].uv[1] = y * 0.5 + 0.5
...
class InfiniteGridMenu {
  ...
  #init(onInit) {
    ...
    this.itemOffset = (this.items.length - (nearestVertexIndex % Math.max(1, this.items.length))) % Math.max(1, this.items.length)
  ...
  #initTexture() {
    ...
    Promise.all(
      ...
      images.forEach((img, i) => {
        ...
        const shouldBw = item.bw ?? true
        ctx.filter = shouldBw ? 'grayscale(1) contrast(1.08)' : 'none'
  ...
  #render() {
    ...
    gl.uniform1i(this.discLocations.uItemOffset, this.itemOffset || 0)
  ...
  #onControlUpdate(deltaTime) {
    ...
    if (!this.control.isPointerDown) {
      const nearestVertexIndex = this.#findNearestVertexIndex()
      const itemIndex = (nearestVertexIndex + (this.itemOffset || 0)) % Math.max(1, this.items.length)
...
const defaultItems = [
  { image: 'https://picsum.photos/900/900?grayscale', link: '/components', title: 'Plugins', description: 'Browse required plugins.' },
]
...
export default function InfiniteMenu({ items = [], scale = 1.0 }) {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index) => {
      const pool = items.length ? items : defaultItems
      const itemIndex = index % pool.length
      setActiveItem(pool[itemIndex])
    }
    if (canvas) {
      try {
        sketch = new InfiniteGridMenu(canvas, items.length ? items : defaultItems, handleActiveItem, setIsMoving, (sk) => sk.run(), scale)
      } catch (error) {
        console.warn('InfiniteMenu init failed:', error)
      }
    }
    const handleResize = () => {
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {activeItem && (
        <>
          <div className="mobile-face-label" aria-live="polite">
            <p className="mobile-face-label-title">{activeItem.title}</p>
          </div>
          <h2 className={`face-title ${isMoving ? 'inactive' : 'active'}`}>{activeItem.title}</h2>
          <p className={`face-description ${isMoving ? 'inactive' : 'active'}`}>{activeItem.description}</p>
          <button type="button" onClick={handleButtonClick} className={`action-button ${isMoving ? 'inactive' : 'active'}`} aria-label="Open item">
